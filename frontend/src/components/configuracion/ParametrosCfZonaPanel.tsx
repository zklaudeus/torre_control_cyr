...
import type { ParametrosCfZona } from '../../types/parametrosConfiguracion';

interface ParametrosCfZonaPanelProps {
  data: ParametrosCfZona[];
  onChange: (zona: string, field: keyof ParametrosCfZona, value: any) => void;
}

export const ParametrosCfZonaPanel = ({ data, onChange }: ParametrosCfZonaPanelProps) => {
  const inputStyle = {
    width: '100%',
    padding: '0.25rem',
    background: '#FFFFFF',
  ...
  return (
    <AccordionPanel title="PARÁMETROS CF POR ZONA" defaultOpen={true}>
      <div style={{ width: '100%', overflowX: 'auto', padding: '1rem' }}>
        <table style={tableStyle}>
          <thead>
            <tr style={{ ...tableHeadRowStyle, background: '#F8FAFC', fontSize: '0.65rem' }}>
              <th style={{ ...thStyle, textAlign: 'left', color: '#f59e0b' }}>ZONA CF</th>
