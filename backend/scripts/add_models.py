...
if "BigInteger" not in content:
    content = content.replace(
        ...
        "from sqlalchemy import Column, Integer, BigInteger, Numeric, String, Boolean, Date, Time, Text, DateTime, text, UniqueConstraint, CheckConstraint, Index"
    )
elif "Numeric" not in content:
    content = content.replace("BigInteger", "BigInteger, Numeric, Index")
...
class RendimientoTecnicoDiario(Base):
    ...
    cumplimiento_pct = Column(Numeric(5, 2), nullable=False)
    ...
    __table_args__ = (
        ...
        Index('idx_rendimiento_fecha_sap', 'fecha_operacional', 'codigo_sap'),
...
class RendimientoTecnicoAusencia(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_ausencia_sap_fecha', 'codigo_sap', 'fecha_operacional'),
...
if "RendimientoTecnicoDiario" not in content:
