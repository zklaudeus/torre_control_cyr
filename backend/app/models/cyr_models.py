...
class ControlParametrosZona(Base):
    ...
    meta_diaria_cortes = Column(Integer, default=30)
    meta_acumulada_09 = Column(Integer, default=5)
    meta_acumulada_10 = Column(Integer, default=10)
    meta_acumulada_11 = Column(Integer, default=15)
    meta_acumulada_12 = Column(Integer, default=20)
    meta_acumulada_13 = Column(Integer, default=25)
    meta_acumulada_14 = Column(Integer, default=30)
    hora_inicio = Column(Time, default='08:00')
    hora_cierre = Column(Time, default='18:00')
    created_at = Column(DateTime(timezone=True), server_default=text('CURRENT_TIMESTAMP'))
    updated_at = Column(DateTime(timezone=True), server_default=text('CURRENT_TIMESTAMP'), onupdate=text('CURRENT_TIMESTAMP'))
...
class ControlParametrosGenerales(Base):
    ...
    hora_cierre_jornada = Column(Time, default='18:00')
    hora_corte_gps = Column(Time, default='16:00')
    meta_diaria_reconexiones = Column(Integer, default=15)
    tramo_horario_inicial = Column(Time, default='09:00')
    tramo_horario_final = Column(Time, default='14:00')
    alerta_sin_brigadas = Column(Boolean, default=True)
    alerta_brigadas_efectivas = Column(Boolean, default=True)
    calcular_cumplimiento_carga = Column(Boolean, default=True)
    calcular_promedio_cortes = Column(Boolean, default=True)
    calcular_promedio_reconexiones = Column(Boolean, default=True)
    calcular_total_actividades = Column(Boolean, default=True)
    calcular_cumplimiento_promedio = Column(Boolean, default=True)
    generar_observacion_automatica = Column(Boolean, default=True)
    activo = Column(Boolean, default=True)
    created_at = Column(DateTime(timezone=True), server_default=text('CURRENT_TIMESTAMP'))
    updated_at = Column(DateTime(timezone=True), server_default=text('CURRENT_TIMESTAMP'), onupdate=text('CURRENT_TIMESTAMP'))
